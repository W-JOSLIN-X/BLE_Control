...
static void *ble_uart_server_task(const char *arg)
{
    ...
    while (1) {
        ...
        osal_printk("BLE Received (%d bytes): %.*s\r\n", msg_data.value_len, msg_data.value_len, msg_data.value);
        ble_uart_server_send_input_report(msg_data.value, 20);
...
#endif /* CONFIG_SAMPLE_SUPPORT_BLE_UART_CLIENT */
    if (task_handle != NULL) {
        osal_kthread_set_priority(task_handle, BLE_UART_TASK_PRIO);
    }
    osal_kthread_unlock();
}
