...
static void ble_uart_add_tx_characters_and_descriptors(uint8_t server_id, uint16_t srvc_handle)
{
    ...
    character.value_len = 20 ; //sizeof(characters_value)
    ...
    descriptor.value_len = 20 ; //sizeof(ccc_val);
...
static void ble_uart_receive_write_req_cbk(uint8_t server_id, uint16_t conn_id, gatts_req_write_cb_t *write_cb_para,
                                           errcode_t status)
{
    if (write_cb_para->length == strlen("GO") &&  write_cb_para->value[0] == 'G' &&  write_cb_para->value[1] == 'O' ) {
        left_wheel_set(500, 500, 1);
        right_wheel_set(500, 500,1);
        osal_printk("CAR is going!\n\r\n");

    }
    if (write_cb_para->length== strlen("BACK") && write_cb_para->value[0] == 'B' && write_cb_para->value[1] == 'A' && write_cb_para->value[2] == 'C'&& write_cb_para->value[3] == 'K') {
        left_wheel_set(500, 500, false);
        right_wheel_set(500, 500,false);
        osal_printk("CAR is BACK!\n\r\n");

    }    
    if (write_cb_para->length== strlen("LEFT") && write_cb_para->value[0] == 'L' && write_cb_para->value[1] == 'E' && write_cb_para->value[2] == 'F' && write_cb_para->value[3] == 'T' ) {
        left_wheel_set(500, 500, false);
        right_wheel_set(500, 500,true);
        osal_printk("CAR is going!\n\r\n");

    }
    if (write_cb_para->length== strlen("RIGHT") && write_cb_para->value[0] == 'R' && write_cb_para->value[1] == 'I' && write_cb_para->value[2] == 'G'&& write_cb_para->value[3] == 'H'&&write_cb_para->value[4] == 'T') {
        left_wheel_set(500, 500, true);
        right_wheel_set(500, 500,false);
        osal_printk("CAR is BACK!\n\r\n");
    }
    if (write_cb_para->length== strlen("STOP") &&write_cb_para->value[0] == 'S' && write_cb_para->value[1] == 'T' && write_cb_para->value[2] == 'O'&&write_cb_para->value[3] == 'P') {
